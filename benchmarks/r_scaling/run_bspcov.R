...
parse_arguments <- function(values) {
  ...
  required <- c(
    "fixture-dir", "fixture-sha256", "burnin", "n-samples", "n-chains",
    "repetitions", "seed"
  )
...
repetitions <- as.integer(arguments[["repetitions"]])
seed <- as.integer(arguments[["seed"]])

expected_draws <- n_samples * chain_count
dimension <- ncol(x)

measure_repetition <- function(repetition_index) {
  ...
  total_seconds <- as.numeric(fit_wall_seconds)
  normalized_seconds <- total_seconds / chain_count

  list(
    repetition = repetition_index,
    seed = repetition_seed,
    raw_wall_seconds = list(total_seconds),
...
result <- list(
  ...
  measured_repetitions = lapply(seq_len(repetitions) - 1L, measure_repetition)
)
